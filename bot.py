...
# Use a Railway Volume for persistence if available: set DB_PATH=/data/counting.db
DB_PATH = os.getenv("DB_PATH", "counting_fun.db")

# Themes (for emoji vibes)
THEMES = {
...
def pick_banter(category: str) -> str:
    lines = BANTER.get(category, [])
...
def get_state(gid: int):
    with db() as conn:
        ...
        if not row:
            conn.execute("""
                INSERT INTO guild_state (guild_id, facts_on)
                VALUES (?, ?)
            """, (gid, 1 if FACTS_ON_DEFAULT else 0))
            row = conn.execute("SELECT * FROM guild_state WHERE guild_id=?", (gid,)).fetchone()
        return row
...
def bump_ok(gid: int, uid: int):
    with db() as conn:
        state = get_state(gid)
        next_num = state["current_number"] + 1
        new_streak = state["guild_streak"] + 1
        best = max(new_streak, state["best_guild_streak"])
        conn.execute("""
        UPDATE guild_state
...
def extract_int(text: str, strict: bool):
    rgx = INT_STRICT if strict else INT_LOOSE
    m = rgx.match(text)
...
def maths_fact(n: int) -> str | None:
    bits = []
    if is_prime(n): bits.append("prime")
    if is_palindrome(n): bits.append("palindrome")
    if n % 10 == 0: bits.append("multiple of 10")
    if n % 100 == 0: bits.append("century number")
    if funny_number(n): bits.append("funny")
    if not bits: return None
    return f"Fun fact: **{n}** is {', '.join(bits)}."
...
# ========= Random Giveaway (hidden targets) =========
...
async def on_message(message: discord.Message):
    ...
    if not state["channel_id"] or message.channel.id != state["channel_id"]:
        return

    strict = bool(state["numbers_only"])
    posted = extract_int(message.content, strict=strict)
    if posted is None:
        # ignore chat in counting channel if it doesn't start with a number
        return

    expected = state["current_number"] + 1
    last_user = state["last_user_id"]

    # no double-post by same user
    if last_user == message.author.id:
        mark_wrong(message.guild.id, message.author.id)
        with contextlib.suppress(Exception):
            await message.add_reaction(theme_emoji(state, "block"))
    ...
    # must be exact next number
    if posted != expected:
        mark_wrong(message.guild.id, message.author.id)
        with contextlib.suppress(Exception):
            await message.add_reaction(theme_emoji(state, "oops"))
    ...
    with contextlib.suppress(Exception):
        await message.add_reaction(theme_emoji(state, "ok"))

    # log correct count for giveaway pool
    with contextlib.suppress(Exception):
        log_correct_count(message.guild.id, expected, message.author.id)

    # milestones (optional fun)
    if expected in MILESTONES:
        theme = THEMES.get(state["theme"], THEMES[DEFAULT_THEME])
        banner = theme["banner"]
        em = discord.Embed(
            title=banner,
    ...
    if add_badge or state["facts_on"]:
    ...
    # arm target if missing, then draw if we hit it
...
class FunCounting(commands.Cog):
    ...
    async def giveaway_config(self, interaction: discord.Interaction,
        ...
        await interaction.response.send_message(
            f"🎰 Giveaway armed. Range **{range_min}–{range_max}** steps • Prize: **{prize}**.\n"
            f"(Target is secret.)", ephemeral=True
        )

    @app_commands.command(name="giveaway_status", description="Peek secret giveaway info (admins only).")
    @app_commands.guild_only()
    async def giveaway_status(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            ...
            f"- Since last jackpot at **{st['last_giveaway_n']}**\n"
            f"- Prize: **{st['giveaway_prize']}**\n"
            f"- ~Next in about **{left}** steps", ephemeral=True
        )
    ...
    async def giveaway_now(self, interaction: discord.Interaction, window: int = 80):
        ...
        with db() as conn:
            st = conn.execute("""
            SELECT current_number, giveaway_prize FROM guild_state WHERE guild_id=?
            """,(gid,)).fetchone()
    ...
    async def reload_banter(self, interaction: discord.Interaction):
        ...
        global BANTER
        BANTER = load_banter()
        await interaction.response.send_message("✅ Banter reloaded from banter.json.", ephemeral=True)
